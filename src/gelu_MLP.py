...
WEEKLY_FREQ = "W-FRI"
RESULTS_DIR = Path("results_mlp_gelu")
NEAR_ZERO_STD = 1e-12
SCENARIO_SAMPLES = 5
...
def plot_predictions(results_df, horizon):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(RESULTS_DIR / f"mlp_predictions_h{horizon}.png", dpi=300)
    plt.close()
...
def plot_residuals(results_df, horizon):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(RESULTS_DIR / f"mlp_residuals_h{horizon}.png", dpi=300)
    plt.close()
...
def plot_reconstructed_levels(results_df, price_df, horizon):
    ...
    merged.to_csv(
        RESULTS_DIR / f"mlp_levels_h{horizon}.csv",
        index=False
    )
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(RESULTS_DIR / f"mlp_levels_h{horizon}.png", dpi=300)
    plt.close()
...
def walkforward_pipeline(args):
    ...
    results_path = RESULTS_DIR / "mlp_rolling_predictions.csv"
    ...
    metrics_path = RESULTS_DIR / "mlp_rolling_metrics.csv"
